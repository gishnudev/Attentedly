...
mongoose.connect('mongodb://mongodb:27017/Attandance_Management')
...
Route.put('/updateLeave/:id', authenticate,async  (req, res) => {
  ...
  // Route: Check attendance for a specific employee and date
Route.get('/attendance/:employeeId/:date', async (req, res) => {
    ...
    try {
      ...
      if (record) {
        ...
        res.status(404).json({ exists: false, message: 'Attendance not found for the given date.' });
      }
    } catch (error) {
      console.error(error);
